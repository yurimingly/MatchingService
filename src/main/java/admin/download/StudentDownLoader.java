...
public class StudentDownLoader extends AbstractXlsView
{
			...
			HttpServletResponse response) throws Exception 
	{
		...
		List<StudentDto> pageRanks = (List<StudentDto>)model.get("pageRanks");
		int rowNum = 1;
		for(StudentDto student : pageRanks)
		{
			createPageRankRow(sheet, student, rowNum++);
		}
	}
...
private void createPageRankRow(Sheet sheet,StudentDto student,int rowNum) 
{
    	Row row=sheet.createRow(rowNum);
    	Cell cell= row.createCell(0);
    	cell.setCellValue(student.getS_num());
    	
    	cell = row.createCell(1);
		cell.setCellValue(student.getName());
		
		cell=row.createCell(2);
		cell.setCellValue(student.getS_email());
		
		cell=row.createCell(3);
		cell.setCellValue(student.getS_phone());
